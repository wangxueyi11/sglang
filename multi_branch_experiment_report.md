...
在多分支推测解码中：
- **接受率** = 被接受的 token 数 / 总 draft token 数
- **接受长度** = 每次推测接受的平均 token 数

对于多分支树，"接受率"的计算存在天然劣势：
- 单分支：8 个 token 在 1 条路径上，接受 3 个 → 接受率 37.5%
- 多分支：8 个 token 分布在多条路径，仍然接受 3 个 → 接受率可能更低

但关键洞察是：**多分支找到了正确的路径！** 即使"接受率"看起来更低，但：
- 每次推测接受更多 token（接受长度更高）
- 吞吐量更高（这才是最终目标）
...
| 样本数 | 30 |
| 最大输出Token数 | 100 |
| draft_tokens 选项 | [4, 8, 12, 16] |
| branch_factor 选项 | [1, 2, 3, 5] |

## 2. 完整矩阵实验结果

### 2.1 吞吐量矩阵 (tokens/sec)

| draft\branch | branch=1 | branch=2 | branch=3 | branch=5 |
|-------------|----------|----------|----------|----------|
| draft=4 | 68.9 (2.10x) | 61.2 (1.86x) | 61.3 (1.87x) | 62.0 (1.89x) |
| draft=8 | 69.6 (2.12x) | **79.8 (2.43x)** | 79.8 (2.43x) | 76.0 (2.31x) |
| draft=12 | 69.1 (2.10x) | **90.9 (2.77x)** | 85.6 (2.61x) | 84.3 (2.57x) |
| draft=16 | 68.3 (2.08x) | 80.4 (2.45x) | 71.7 (2.18x) | 76.6 (2.33x) |

### 2.2 平均接受长度矩阵

| draft\branch | branch=1 | branch=2 | branch=3 | branch=5 |
|-------------|----------|----------|----------|----------|
| draft=4 | 2.50 | 2.57 | 2.58 | 2.60 |
| draft=8 | 2.51 | **3.43** | 3.47 | 3.24 |
| draft=12 | 2.49 | **3.98** | 3.71 | 3.71 |
| draft=16 | 2.48 | 3.45 | 3.11 | 3.15 |

### 2.3 关键发现

#### 发现1：draft=4 时多分支反而更差
- **原因**：树太小（只有4个token），多分支分散后每个分支太短
- **结论**：draft ≤ 4 时，应使用单分支模式

#### 发现2：branch=2 是最优分支数
- 在 draft=8,12,16 时，branch=2 都达到最高吞吐量
- branch=3,5 的额外验证开销超过了收益

#### 发现3：接受长度是更好的指标
- draft=12, branch=2：接受长度 3.98，吞吐量 90.9
- draft=12, branch=1：接受长度 2.49，吞吐量 69.1
- 接受长度提升 60%，吞吐量提升 31%
...
从日志确认多分支树确实在构建：
```
[MULTI-BRANCH] roots=5, multi_child_parents={-1: 5, 1: 2}
[SUFFIX DEBUG DEEP TREE] parents=[-1, -1, -1, -1, -1, 0, 1, 1], max_branches=2
```
...
| 场景 | 推荐配置 | 吞吐量 | 加速比 | 接受长度 |
|------|---------|--------|--------|----------|
| **追求吞吐量** | draft=12, branch=2 | 90.9 | 2.77x | 3.98 |
| **平衡选择** | draft=8, branch=2 | 79.8 | 2.43x | 3.43 |
| **小树场景** | draft=4, branch=1 | 68.9 | 2.10x | 2.50 |
